package im.grusis.mkb.core.emulator;

/**
 * User: Mothership
 * Date: 13-6-15
 * Time: 下午7:37
 */
public class ItemCode {

  public static final int Tutorial_Fight = 1;
  public static final int[] Tutorial_Fight_Stages = {21};
  public static final int Tutorial_Card = 2;
  public static final int[] Tutorial_Card_Stages = {10, 16};
  public static final int Tutorial_Streng = 3;
  public static final int[] Tutorial_Streng_Stages = {17};
  public static final int Tutorial_Rune = 4;
  public static final int[] Tutorial_Rune_Stages = {5, 13};
  public static final int Tutorial_Muse = 5;
  public static final int[] Tutorial_Muse_Stages = {16};
  public static final int Tutorial_Fragment = 6;
  public static final int[] Tutorial_Fragment_Stages = {5};
  public static final int Tutorial_Maze1 = 7;
  public static final int[] Tutorial_Maze1_Stages = {7};
  public static final int Tutorial_Maze2 = 8;
  public static final int[] Tutorial_Maze2_Stages = {3};
  public static final int Tutorial_Arena = 9;
  public static final int[] Tutorial_Arena_Stages = {9};
  public static final int Tutorial_Cost = 10;
  public static final int[] Tutorial_Cost_Stages = {4};

  public static final int Melee_Coins = 1;
  public static final int Melee_Cash = 2;

  public static final int Coins = 1;
  public static final int Cash = 2;
  public static final int Ticket = 3;
  public static final int Card = 4;
}
